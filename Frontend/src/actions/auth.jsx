...
export const googleAuthenticate = (state, code,ShowToast) => async dispatch => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_APP_API_URL}/auth/o/google-oauth2/`,
        {
          method: 'POST',
          credentials: 'include',
          headers: {
...
export const delete_user = (email,password) =>  async dispatch => {
    ...
    if (localStorage.getItem('access')  != 'undefined'){
        ...
        try {
            ...
            //const res = await axios.post(`${import.meta.env.VITE_APP_API_URL}/auth/jwt/create/`,config, body );
            fetch(`${import.meta.env.VITE_APP_API_URL}/app/delete_account/`, requestOptions)
            .then(response => response.text())
            .then(result => LoaderResponse(result))
            .catch(error => {
                //console.error('There has been a problem with your fetch operation:', error);
...
export const resend_verification = (email, navigate,ShowToast) => async dispatch => {
    ...
    try {
        ...
        const response = await fetch(
            `${import.meta.env.VITE_APP_API_URL}/auth/users/resend_activation/`, 
            requestOptions
        );
